...
const components = {
  ...
  YoutubeEmbed,
  img: (props: any) => (
    <span style={{ display: 'block', margin: '2rem 0', borderRadius: '16px', overflow: 'hidden', boxShadow: 'var(--shadow-lg)' }}>
...
export async function generateStaticParams() {
  const slugs = getPostSlugs("gallery");
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const post = getPostBySlug("gallery", resolvedParams.slug);
    return {
      title: `${post.metadata.title} | AI 이미지 갤러리`,
...
export default async function GalleryPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  let post;
  try {
    post = getPostBySlug("gallery", resolvedParams.slug);
  } catch (e) {
    notFound();
  }
    ...
    <div className={styles.layout}>
      <div className={styles.mainContainer} style={{ maxWidth: "800px" }}>
        <div style={{ marginBottom: "2rem" }}>
          <Link href="/gallery" style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", color: "var(--fg-muted)", textDecoration: "none", fontSize: "0.95rem" }}>
        ...
        <div style={{ width: "100%", borderRadius: "24px", overflow: "hidden", marginBottom: "3rem", background: "var(--bg-subtle)", boxShadow: "var(--shadow-xl)" }}>
