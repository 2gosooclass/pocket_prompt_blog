...
export async function generateStaticParams() {
  const slugs = getPostSlugs("lectures");
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const post = getPostBySlug("lectures", resolvedParams.slug);
    return {
      title: `${post.metadata.title} | 2GOSOO AI 교육`,
...
export default async function LecturePostPage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = await params;
  let post;
  try {
    post = getPostBySlug("lectures", resolvedParams.slug);
  } catch (e) {
    notFound();
  }
    ...
    <div className={styles.layout}>
      <div className={styles.mainContainer} style={{ maxWidth: "800px" }}>
        <div style={{ marginBottom: "2rem" }}>
          <Link href="/lectures" style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", color: "var(--fg-muted)", textDecoration: "none", fontSize: "0.95rem" }}>
