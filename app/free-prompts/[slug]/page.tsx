...
export async function generateStaticParams() {
  const slugs = getPostSlugs("free-prompts");
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  try {
    const post = getPostBySlug("free-prompts", resolvedParams.slug);
    return {
      title: `${post.metadata.title} | 무료 프롬프트`,
...
export default async function FreePromptPostPage({ params }: { params: Promise<{ slug: string }> }) {
  const resolvedParams = await params;
  let post;
  try {
    post = getPostBySlug("free-prompts", resolvedParams.slug);
  } catch (e) {
    notFound();
  }
    ...
    <div className={styles.layout}>
      <div className={styles.mainContainer} style={{ maxWidth: "800px" }}>
        <div style={{ marginBottom: "2rem" }}>
          <Link href="/free-prompts" style={{ display: "inline-flex", alignItems: "center", gap: "0.5rem", color: "var(--fg-muted)", textDecoration: "none", fontSize: "0.95rem" }}>
